...
public class QuorumPeer extends ZooKeeperThread implements QuorumStats.Provider {
    ...
    public synchronized void start() {
         ...
         * 当前类,quorumPeer继承了ZKThread,它本身就是一个线程类,super.start();就是启动它的run方法,
         * 在他的Run方法中有一个while循环,一开始在程序启动的阶段,所有的节点的默认值都是Looking,
         * 于是会进入这个分支中,在这个分之中会进行真正的leader选举工作
         */
        startLeaderElection();
        /**
         * 调用一个线程
         * 确定服务器的角色, 启动的就是当前类的run方法
         */
        super.start();
    }
    ...
    private void loadDataBase() {
        try {
            ...
            /**
             * 从最新的zxid恢复epoch变量
             * zxid 64位，前32位是epoch的值，后32位是zxid
             */
            long lastProcessedZxid = zkDb.getDataTree().lastProcessedZxid;
            long epochOfZxid = ZxidUtils.getEpochFromZxid(lastProcessedZxid);
            try {
                /**
                 * 从文件中读取当前的epoch(当前的leader任期)
                 */
...
//        }
    ...
    /**
     * 创建了  QuorumCnxManager
     * 创建了  listener
     * 创建了  FastLeaderElection
     */
    ...
    protected Election createElectionAlgorithm(int electionAlgorithm){
        ...
        case 3:
            ...
            if(listener != null){
                /**
                 * 启动已绑定的端口的选举线程，等待集群中的其他线程
                 */
                listener.start();
                /**
                 * 实例化领导者选举的算法 ——> 基于TCP的选举算法
                 */
